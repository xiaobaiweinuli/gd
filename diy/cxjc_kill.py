...
async def pidkill(event):
    try:    
        ...
        for message in messages:
            ...
            if isdokill==0:
                await jdbot.send_message(chat_id, f'进程结束失败: 当前系统未查询到该pid {killpid}')
                return
                ...
                for line in txt:
                    ...
                    if "node" in line:
                        ...
                        if killpid==pid:
                            ...
                            break
            if isdokill==0:
                await jdbot.send_message(chat_id, f'进程{killpid}已被强制结束!')
            else:
                await jdbot.send_message(chat_id, f'进程{killpid}强制结束失败!')
