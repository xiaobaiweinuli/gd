    ...
    try:
        ...
        async with jdbot.conversation(SENDER, timeout=60) as conv:
            msg = await conv.send_message(
                "您是否更新要[curtinlv/gd](https://github.com/curtinlv/gd.git)库的监控",
                buttons=btns,
                link_preview=False,
            )
            ...
            if res == 'cancel':
                ...
                if newversion:
                    ...
                    msg = await jdbot.edit_message(msg, "获取新版本失败~")
                    conv.cancel()
                    return
            conv.cancel()
        if V4:
            ...
            await jdbot.delete_messages(chat_id, msg)
        elif '下载代理' in BOT_SET.keys() and str(BOT_SET['下载代理']).lower() != 'false':
